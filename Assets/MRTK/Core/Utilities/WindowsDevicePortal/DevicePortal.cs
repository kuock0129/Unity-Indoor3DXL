...
using AwaiterExtensions = Microsoft.MixedReality.Toolkit.Utilities.AwaiterExtensions;
...
namespace Microsoft.MixedReality.Toolkit.WindowsDevicePortal
{
    ...
    public static class DevicePortal
    {
        ...
        private static async Task<Response> DevicePortalAuthorizationAsync(DeviceInfo targetDevice)
        {
            ...
            //await AwaiterExtensions.GetAwaiter(webRequest.SendWebRequest());
            await AwaitWebRequest(webRequest);
        ...
        private static Task AwaitWebRequest(UnityWebRequest webRequest)
        {
            ...
            // Create a TaskCompletionSource to bridge between Unity's AsyncOperation and Task
            var tcs = new TaskCompletionSource<bool>();

            // Register a callback for when the operation completes
            asyncOp.completed += op => {
                tcs.SetResult(true);
            };

            // Return the task that will complete when the operation completes
            return tcs.Task;
        }
